...
export default function CornerSetupScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (active) {
        ...
        setDraft(
          outs && outs.length === 4 ? [...outs] : defaultOutsForType("corner"),
        );
  ...
  async function changeRevealKind(kind: GimmickRevealKind) {
    ...
    setSaved(false);
    if (!note) return;
    // Persist immediately so reveals use the right mode even before "Save".
  ...
  return (
    <SafeAreaView
      ...
      edges={["top"]}
    >
      <ScreenHeader title="Corner Setup" />
      <ScrollView
      ...
      >
        <Text style={[styles.subtitle, { color: theme.icon }]}>
          Outs for {note.title}. Edit what each corner tap reveals.
        </Text>
